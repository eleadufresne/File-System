 ...
 */
#define MAX_FNAME_LENGTH 20 //32  /* Assume at most 20 characters (16.3) */
...
main(int argc, char **argv)
{
   ...
   */
  //printf("-----First we open two files and attempt to write data to them.--------\n");
  for (i = 0; i < 2; i++) {
    names[i] = rand_name();
  ...
  //printf("-----Sneaky attempt to close already closed file handle. --------\n");
  if (sfs_fclose(fds[1]) == 0) {
   ...
   */
  //printf("-----Just to be cruel - attempt to read from a closed file handle. --------\n");
   ...
   */
  //printf("-----Now we try to close the files.--------------------\n");
  for (i = 0; i < 2; i++) {
    if (sfs_fclose(fds[i]) == 0) {
   ...
   */
  //printf("----Now just try to open up a bunch of files.-----------------\n");
  ncreate = 0;
  for (i = 0; i < MAX_FD; i++) {
    names[i] = rand_name();
    fds[i] = sfs_fopen(names[i]);
  ...
  //printf("----Re-open in reverse order.-----------------%d\n", nopen);
  for (i = nopen-1; i >= 0; i--) {
    fds[i] = sfs_fopen(names[i]);
    if (fds[i] < 0) {
   ...
   */
 // printf("----Now test the file contents.-----------------\n");
  for (i = 0; i < nopen; i++) {
      sfs_fseek(fds[i], 0);
  }
   ...
   */
    //printf("----Now close all of the open file handles.-----------------\n");
  for (i = 0; i < nopen; i++) {
    if (sfs_fclose(fds[i]) != 0) {
      fprintf(stderr, "ERROR: close of handle %d failed\n", fds[i]);
      error_count++;
  ...
  printf("ok\n");
  //sfs_print_i_node_table();

  /* Now we try to re-initialize the system.
   */
  //printf("----Now we try to re-initialize the system.----------------\n");
  ...
  for (i = 0; i < nopen; i++) {
    ...
    if (fds[i] >= 0) {
      ...
      if (readsize != strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes, expected %ld, and got %d\n", strlen(test_str), readsize);
        error_count++;
      }
